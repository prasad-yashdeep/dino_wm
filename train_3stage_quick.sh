...
################################################################################
# 3-STAGE TRAINING PIPELINE - QUICK TEST
################################################################################
# This script runs the complete 3-stage training pipeline with reduced epochs
# and smaller dataset for quick testing and validation.
################################################################################
...
# Training hyperparameters (QUICK TEST -> MEDIUM TEST) #edited by B
DATASET_ROLLOUTS=1920 # Increased from 100 for larger codebooks #edited by B
STATE_CODEBOOK_SIZE=64  # Increased from 16 for more capacity #edited by B
...
$PYTHON train.py \
    ...
    disable_wandb=false\
    force_restart=true \
    2>&1 | tee stage1_quick.log

# Extract checkpoint directory from log
STAGE1_DIR=$(grep "Model saved dir:" stage1_quick.log 2>/dev/null | tail -1 | sed 's/.*Model saved dir: //')

if [ -z "$STAGE1_DIR" ]; then
    echo "❌ Error: Could not find Stage 1 checkpoint directory"
    echo "Check stage1_quick.log for errors"
    exit 1
fi
...
$PYTHON train.py \
    ...
    2>&1 | tee stage2_quick.log  #edited by B

# Extract checkpoint directory from log
STAGE2_DIR=$(grep "Model saved dir:" stage2_quick.log 2>/dev/null | tail -1 | sed 's/.*Model saved dir: //')

if [ -z "$STAGE2_DIR" ]; then
    echo "❌ Error: Could not find Stage 2 checkpoint directory"
    echo "Check stage2_quick.log for errors"
    exit 1
fi
...
$PYTHON train.py \
    ...
    2>&1 | tee stage3_quick.log  #edited by B

# Extract checkpoint directory from log
STAGE3_DIR=$(grep "Model saved dir:" stage3_quick.log 2>/dev/null | tail -1 | sed 's/.*Model saved dir: //')

if [ -z "$STAGE3_DIR" ]; then
    echo "❌ Error: Could not find Stage 3 checkpoint directory"
    echo "Check stage3_quick.log for errors"
    exit 1
fi
...
echo ""
echo "Logs saved:"
echo "  Stage 1: $(pwd)/stage1_quick.log"
echo "  Stage 2: $(pwd)/stage2_quick.log"
echo "  Stage 3: $(pwd)/stage3_quick.log"
echo ""
echo "To visualize:"
...
grep "State Codebook:" stage3_quick.log | tail -1 || echo "  (check stage3_quick.log)"
grep "Action Codebook:" stage3_quick.log | tail -1 || echo "  (check stage3_quick.log)"
echo ""
echo "Final collapse loss:"
grep "z_collapse_loss:" stage3_quick.log | tail -1 || echo "  (check stage3_quick.log)"
echo ""
echo "========================================================================"
